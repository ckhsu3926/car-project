...
CREATE TABLE IF NOT EXISTS `car-project`.`vehicle` (
  `id` SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '車輛ID',
  ...
  PRIMARY KEY (`id`),
  INDEX `INDEX_VEHICLE_USER` USING BTREE (`user_id`))
ENGINE = InnoDB
COMMENT = '車輛';
...
CREATE TABLE IF NOT EXISTS `car-project`.`vehicle_insurance` (
  ...
  `insurance_uid` VARCHAR(20) NOT NULL COMMENT '保單號碼',
  `name` VARCHAR(45) NOT NULL COMMENT '保單名稱',
  `company` VARCHAR(45) NOT NULL COMMENT '保險公司',
  ...
  PRIMARY KEY (`vehicle_id`, `insurance_uid`))
ENGINE = InnoDB
COMMENT = '車輛保險';
...
CREATE TABLE IF NOT EXISTS `car-project`.`insurance` (
  ...
  PRIMARY KEY (`id`),
  INDEX `INDEX_INSURANCE_UID` (`insurance_uid` ASC))
ENGINE = InnoDB
COMMENT = '保單內容';
...
CREATE TABLE IF NOT EXISTS `car-project`.`refueling_record` (
  `id` MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '加油紀錄編號',
  `vehicle_id` SMALLINT UNSIGNED NOT NULL COMMENT '車輛ID',
  ...
  PRIMARY KEY (`id`),
  INDEX `INDEX_REFUELING_VEHICLE` (`vehicle_id` ASC))
ENGINE = InnoDB
COMMENT = '加油紀錄';
...
CREATE TABLE IF NOT EXISTS `car-project`.`maintenance_record` (
  `id` MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '維修編號',
  `vehicle_id` SMALLINT UNSIGNED NOT NULL COMMENT '車輛ID',
  ...
  PRIMARY KEY (`id`),
  INDEX `INDEX_MAINTENANCE_VEHICLE` (`vehicle_id` ASC))
ENGINE = InnoDB
COMMENT = '加油紀錄';
...
CREATE TABLE IF NOT EXISTS `car-project`.`maintenance_detail` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '維修項目ID',
  ...
  PRIMARY KEY (`id`),
  INDEX `INDEX_MAINTENANCE_DETAIL_RECORD` (`maintenance_record_id` ASC))
ENGINE = InnoDB
COMMENT = '維修項目';
...
CREATE TABLE IF NOT EXISTS `car-project`.`user_gas_station` (
  ...
  PRIMARY KEY (`id`),
  INDEX `INDEX_GAS_USER` (`user_id` ASC))
ENGINE = InnoDB;
